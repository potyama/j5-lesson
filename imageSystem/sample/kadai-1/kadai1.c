...
int main(int argc, char *argv[])
{
  ...
  //   その他、var.h と def.h を見て下さい
    ...
    if (argc < 3) printf("使用法：cpbmp コピー元.bmp コピー先.bmp\n");
        ...
        /* 課題1 : 入力画像をモノクロ化するプログラム */
        for (y = 0; y < idata.height; y++){
	        for (x = 0; x < idata.width; x++){
                double gamma = idata.source[RED][y][x]*0.299 + idata.source[GREEN][y][x]*0.587 + idata.source[BLUE][y][x]*0.114;
	            idata.results[RED][y][x] = gamma;
	            idata.results[GREEN][y][x]= gamma;
	            idata.results[BLUE][y][x]= gamma;
	        }
        }
