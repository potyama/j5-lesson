...
int main(int argc, char *argv[])
{
  ...
  //   その他、var.h と def.h を見て下さい
    ...
    if (argc < 3) printf("使用法：kadai1 コピー元.bmp コピー先.bmp\n");
        ...
        /* 課題1 : 入力画像を濃度反転するプログラム */
        for (y = 0; y < idata.height; y++){
            for (x = 0; x < idata.width; x++){
                idata.results[RED][y][x] = 255 - idata.source[RED][y][x];
                idata.results[GREEN][y][x] = 255 - idata.source[GREEN][y][x];
                idata.results[BLUE][y][x] = 255- idata.source[BLUE][y][x];
            }
        }
