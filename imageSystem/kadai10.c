...
int bsort(double c[], int cnt){
    ...
    if(cnt% 2 == 0){
        return (c[cnt/2] +c[((cnt/2)+1)/2] );
    }

    return c[(cnt/2) + 1];
}

int mean_value_filter(imgdata idata, int x, int y, int h_start, int h_stop, int w_start,int w_stop){
    double sum_data = 0;
    int cnt = 0;
    double data[12] = {};
    ...
    if (argc < 3) printf("使用法：kadai1 コピー元.bmp コピー先.bmp\n");
        ...
        for (int y = 0; y < idata.height; y++){
            for (int x = 0; x < idata.width; x++){
                ...
                if(x == 0){
                    if(y == 0){
                        sum_data = mean_value_filter(idata, x, y, 0, 1, 0, 1);
                    }else if(y == 255){
                        sum_data = mean_value_filter(idata, x, y, -1, 0, 0, 1);
                    }else{
                        sum_data = mean_value_filter(idata, x, y, -1, 1, 0, 1);
                    }
                }else if(x == 255){
                    if(y == 0){
                        sum_data = mean_value_filter(idata, x, y, 0, 1, -1, 0);
                    }else if(y == 255){
                        sum_data = mean_value_filter(idata, x, y, -1, 0, -1, 0);
                    }else{
                        sum_data = mean_value_filter(idata, x, y, -1, 1, -1, 0);
                    }
                }else if(y == 0){
                    sum_data = mean_value_filter(idata, x, y, 0, 1, -1, 1);
                }else if(y == 255){
                    sum_data = mean_value_filter(idata, x, y, -1, 0, -1, 1);
                }else{
